...
function PlannerApp() {
  ...
  const content = data[5]; // Das ist das Objekt mit allen Chat- und App-Konfigurationen
...
const MS_PER_DAY = 1000 * 60 * 60 * 24;
  ...
  const processAIResponse = (content) => {
    const defaultMotivation = content?.chat_defaultMotivation || 'Erreiche dein Ziel!';
    ...
    if (allNewEvents.length > 0) {
      ...
      setTimeout(() => {
        const successMessage = (content?.chat_autoImportSuccess || 'Automatisch {count} Termine importiert!').replace('{count}', allNewEvents.length);
    ...
    for (const file of files) {
      try {
        ...
        if (file.name.endsWith('.ics')) {
          fileType = 'calendar';
          parsedEvents = fileUtils.parseIcsToRoadmapData(fileContent, defaultMotivation);
        } else if (file.name.endsWith('.json')) {
          fileType = 'json';
          parsedEvents = fileUtils.parseJsonToRoadmapData(fileContent, defaultMotivation);
        }
        ...
        setUploadedFiles(prev => [...prev, {
          id: Date.now() + Math.random(), name: file.name, content: fileContent, type: fileType, size: file.size
        }]);
      } catch (error) {
      ...
      <div id="part1" style={{ display: part1 }}>
        ...
        {gesamtPrompt && (
          <div className="active-prompt-display">
            <strong>{content?.chat_activePromptLabel || 'Aktiver Prompt'}:</strong> Ready to generate plan.
          </div>
        )}
      ...
      <div id="part2" style={{ display: part2 }}>
        {roadmapToday.length > 0 ? (
          ...
          <div className="info-box">
            {(content?.chat_noTasksToday || 'No Tasks for today! ({today})').replace('{today}', today)}
          </div>
        )}
      </div>

      <div id="part3" style={{ display: part3 }}>
        <h2>{content?.app_Headline3}</h2>
        <p className="info-box">
          <strong>ℹ️ {content?.chat_infoLabel || 'Info'}:</strong>
          {' '}
          {(content?.chat_roadmapInfo || 'Der Projektplan wird automatisch aktualisiert, wenn die KI Kalenderdaten erstellt. Aktuell werden {count} Termine angezeigt.').replace('{count}', roadmapData.length)}
        </p>
        <RoadmapEdit
